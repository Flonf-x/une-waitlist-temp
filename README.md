...
Crée une base **Neon** gratuite et persistante : https://neon.tech → nouveau projet → copie le *connection string* (`postgresql://…?sslmode=require`) dans `DATABASE_URL`. Le schéma (`schema.sql`) est appliqué automatiquement au démarrage (idempotent) — rien à migrer à la main.
...
```html
<script>window.UNE_API_BASE_URL = "https://REMPLACE-PAR-TON-BACKEND.onrender.com";</script>
```
